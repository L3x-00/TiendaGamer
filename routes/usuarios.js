...
router.post('/', requireRole('super'), async (req, res) => {
  ...
  try {
    ...
    const result = await db.query('INSERT INTO usuarios (username, password, role) VALUES ($1, $2, $3) RETURNING *', [username, hashed, role]);
    res.json({ id: result.rows[0].id, username, role });
  } catch (err) {
    // CAMBIO: Código de error de PostgreSQL para duplicados
